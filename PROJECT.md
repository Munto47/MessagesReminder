...
| 新消息判定 | 优先使用原生 `id` 与 `lastMessageId` 比较；无原生 ID 时退化为稳定指纹判重，避免列表重排导致重复提醒。 |
| 关键词提醒 | 默认关键词为 **「活动开始签到」**，可通过环境变量 `KEYWORD` 调整，支持 `关键词1|关键词2` 的多关键词形式。 |
| 首轮不刷屏 | 仅在**成功解析出消息列表**后才推进 `initialScanDone`，避免解析失败时误把历史消息当作新消息。 |
| Token 告警 | 接口返回 401/403 时经 Server酱 提示更新 Token，并受 `TOKEN_ALERT_COOLDOWN_MS` 节流；仅在告警发送成功后进入冷却。 |
...
| 路径 | 作用 |
|------|------|
| `index.js` | 入口：加载环境变量并启动轮询服务。 |
| `poller.js` | 轮询核心：请求、解密、解析、判重、提醒与 cron 启动。 |
...
- **`SERVERCHAN_BASE_URL`**：可选，自定义 Server酱基础地址；用于本地 mock 调试时可指向本机服务。  
- **`STATE_FILE`**：状态文件路径，默认 `./data/last_state.json`。
- **`KEYWORD`**：可选，提醒关键词；默认 `活动开始签到`，支持 `活动开始签到|活动开始签退` 这样的多关键词配置。
- **`POLL_CRON_EXPR`**：可选，cron 表达式；默认 `*/1 * * * *`。
...
该脚本会启动本地模拟消息接口与模拟通知服务，验证“首轮不推送、第二轮关键词推送、无 ID 消息重排不重复推送、401 告警失败不误冷却”等关键行为。
...
1. 启动后立即执行一轮拉取，之后按 `POLL_CRON_EXPR`（默认每分钟）执行。  
2. 解密成功后从 JSON 中取消息列表；有原生 `id` 的消息走 `lastMessageId` 游标，无原生 ID 的消息走稳定指纹判重。  
3. 仅当消息为“新增”、正文包含任一 **关键词**、且 **`initialScanDone` 已为 true** 时，发送 Server酱。  
4. 每轮结束后仅在**成功解析到消息列表**时推进 `initialScanDone`，并同步更新 `lastMessageId` / `seenMessageFingerprints`。

修改关键词：直接在 `.env` 中设置 `KEYWORD`，多个关键词用 `|` 分隔。
