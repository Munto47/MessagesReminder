...
export const DEFAULT_KEYWORD = "活动开始签到";
export const DEFAULT_CRON_EXPR = "*/1 * * * *";
const MAX_SEEN_MESSAGE_FINGERPRINTS = 500;
 ...
 */
export function resolveEncryptedParts(res) {
...
export function getReminderKeywords(raw = process.env.KEYWORD) {
  const source =
    raw == null || String(raw).trim() === "" ? DEFAULT_KEYWORD : String(raw);
  const parts = source
    .split(/[|\n\r]/)
    .map((item) => item.trim())
    .filter(Boolean);
  return parts.length > 0 ? [...new Set(parts)] : [DEFAULT_KEYWORD];
}
...
export function getNativeMessageId(msg) {
...
export function buildStableMessageFingerprint(msg) {
  const raw = stableSerialize(msg ?? null);
...
const MESSAGE_LIST_CANDIDATES = [
  ...
  { path: "rows", fn: (d) => d?.rows },
];

export function extractMessagesResult(decrypted) {
...
function mergeSeenFingerprints(existing, incoming) {
  ...
  if (merged.length <= MAX_SEEN_MESSAGE_FINGERPRINTS) return merged;
  return merged.slice(-MAX_SEEN_MESSAGE_FINGERPRINTS);
}

async function maybeAlertTokenExpired(stateStore, notifier) {
  ...
  if (
    ...
    return { alerted: false, cooledDown: true };
  }
  ...
  return { alerted: sent, cooledDown: false };
}

export async function runPoll(stateStore, options = {}) {
  const token = process.env.API_BEARER_TOKEN;
  const httpClient = options.httpClient ?? axios;
  const notifier = options.notifier ?? sendNotification;
  const keywords = Array.isArray(options.keywords)
    ? options.keywords
    : getReminderKeywords(options.keyword);
  ...
  if (!resolved) {
    ...
    return { ok: false, reason: "missing_request_config" };
  }
  ...
  let res;
  try {
    res = await httpClient.get(apiUrl, {
      headers: pollHeaders,
      responseType: "text",
      timeout,
      validateStatus: () => true,
  ...
  if (res.status === 401 || res.status === 403) {
    ...
    const tokenAlert = await maybeAlertTokenExpired(stateStore, notifier);
    return { ok: false, reason: "unauthorized", status: res.status, tokenAlert };
  }
  ...
  if (isLikelyHtmlGatewayResponse(res)) {
    ...
    return { ok: false, reason: "html_gateway" };
  }
  ...
  if (!encryptedFlag || encryptedData == null || String(encryptedData).trim() === "") {
    ...
    return { ok: false, reason: "missing_encrypted_parts" };
  }
  ...
  try {
    ...
    return { ok: false, reason: "decrypt_failed", error: err };
  }
  ...
  const parseSucceeded = matchedPath != null;
  const state = await stateStore.read();
  const lastId = state.lastMessageId;
  const allowNotify = state.initialScanDone === true;
  const seenFingerprints = new Set(state.seenMessageFingerprints || []);
  const seenFallbackThisPoll = [];

  const nativeEntries = [];
  const fallbackEntries = [];
  for (const msg of messages) {
    const nativeId = getNativeMessageId(msg);
    if (nativeId != null) {
      nativeEntries.push({ msg, nativeId });
    } else {
      fallbackEntries.push({
        msg,
        fingerprint: buildStableMessageFingerprint(msg),
      });
    }
  }
  nativeEntries.sort((a, b) => compareIdDesc(a.nativeId, b.nativeId));
  ...
  const maybeSendReminder = async (text, details) => {
    ...
    if (ok) sentNotifications += 1;
  };

  for (const { msg, nativeId } of nativeEntries) {
    newMax = maxId(newMax, nativeId);
    if (!idGreaterThan(nativeId, lastId)) continue;
    await maybeSendReminder(getMessageText(msg), `ID: ${nativeId}`);
  }

  for (const { msg, fingerprint } of fallbackEntries) {
    seenFallbackThisPoll.push(fingerprint);
    if (seenFingerprints.has(fingerprint)) continue;
    seenFingerprints.add(fingerprint);
    await maybeSendReminder(getMessageText(msg), `Fingerprint: ${fingerprint}`);
  }

  const patch = {};
  if (parseSucceeded) {
    patch.initialScanDone = true;
    patch.seenMessageFingerprints = mergeSeenFingerprints(
      state.seenMessageFingerprints || [],
      seenFallbackThisPoll
    );
  }
  ...
  if (parseDebug) {
    console.log(
      "[poll] parse: 列表匹配路径:",
      matchedPath ?? "(未匹配 — 请根据下方键名在 poller.js 的 MESSAGE_LIST_CANDIDATES 中补充)"
    );
    ...
    if (messages.length > 0) {
      ...
      console.log(
        "[poll] parse: 首条 nativeId:",
        getNativeMessageId(messages[0]) ?? "(无原生 ID)"
      );
      console.log(
        "[poll] parse: 首条 fingerprint:",
        buildStableMessageFingerprint(messages[0])
      );
    }
    console.log("[poll] parse: 上轮 lastMessageId:", lastId ?? "(空)");
    console.log("[poll] parse: 本轮计算 newMax:", newMax ?? "(空)");
    console.log(
      "[poll] parse: 是否推进 initialScanDone:",
      patch.initialScanDone === true,
      patch.initialScanDone === true ? "→ true" : "(保持不变)"
    );
    if (!parseSucceeded) {
      console.log(
        "[poll] parse: 未推进基线原因: 本轮未匹配到消息数组，避免后续历史消息被误判为新消息。"
      );
    ...
    console.log(
      "[poll] parse: 已记录 fallback 指纹数:",
      (patch.seenMessageFingerprints || state.seenMessageFingerprints || []).length
    );
    console.log(
      "[poll] parse: 关键词匹配的新消息条数:",
      notifyCandidates,
      `（关键词: ${keywords.join(" | ")}）`
    );
    if (skippedByBaseline > 0) {
      console.log(
        "[poll] parse: 首轮不推送，已跳过关键词消息条数:",
        skippedByBaseline
      );
    }
    console.log("[poll] parse: 实际成功发送通知条数:", sentNotifications);
  }
  ...
  return {
    ...
    parseSucceeded,
    statePatch: patch,
    lastMessageId: patch.lastMessageId ?? lastId,
  };
}

export function createStateStore(statePath = process.env.STATE_FILE?.trim() || "./data/last_state.json") {
  return new MessageStateStore(statePath);
}

export async function startPollingService(options = {}) {
  const stateStore = options.stateStore ?? createStateStore();
  const cronExpr = process.env.POLL_CRON_EXPR?.trim() || DEFAULT_CRON_EXPR;
  ...
  const tick = async () => {
    ...
    inFlight = true;
    try {
      await runPoll(stateStore, options);
    } catch (err) {
  ...
  const statePath = stateStore.filePath || process.env.STATE_FILE?.trim() || "./data/last_state.json";
  console.log(
    "[main] 已启动：%s 轮询一次，STATE_FILE=%s",
    cronExpr,
    statePath
  );

  return { job, stateStore };
}
